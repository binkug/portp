...
public class UserServiceImpl implements UserService {
	...
	public void list(HttpServletRequest request, HttpServletResponse response) {
		...
		System.out.println("서비스 리스트 출력 : " + list);
	...
	public void detail(HttpServletRequest request, HttpServletResponse response) {
		...
		String userEmail = request.getParameter("user_email");
		System.out.println("detail 서비스 가져오기" + userEmail);
		// email 중복검사 실행
		String emailResult = null;
	...
	public Map<String, Object> login(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			for(User user : list) {
				if(CryptoUtil.decryptAES256(user.getUser_email(), key).equals(userEmail)&&
						BCrypt.checkpw(userPassword, user.getUser_password())){
					result.put("result", true);
					// "" 안에 들어가는 값은 파라미터로 가져온 값을 넣어주면 된다. 
					result.put("user_email", userEmail);
					result.put("user_name", user.getUser_name());
					result.put("user_image", user.getUser_image());
					break;
				}
			}
		}catch (Exception e) {
